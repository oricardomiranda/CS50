...
def index():
    ...
    transactions = db.execute(
        "SELECT symbol, name, SUM(shares) AS shares, price FROM transactions WHERE user_id = (?) GROUP BY symbol HAVING SUM(shares) > 0;",
        user_id,
    )
...
def history():
    ...
    user_id = session["user_id"]
    transactions = db.execute(
        "SELECT symbol, name, shares, price, Timestamp FROM transactions WHERE user_id = (?);",
        user_id,
    )
...
def sell():
    ...
    oldshares = db.execute(
        "SELECT symbol, SUM(shares) AS shares FROM transactions WHERE symbol = (?);",
        symbol,
    )
